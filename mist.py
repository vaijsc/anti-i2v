...
def transformer_training(
    ...
    if args.precompute_video_latents:
        ...
        video_latents = video_latents.to(device=device, memory_format=torch.contiguous_format, dtype=dtype)
    else:
        if videos is None:
    ...
    image_latents = vae.encode(images_in).latent_dist.sample() * vae.config.scaling_factor
    ...
    loss = torch.mean(
        ((model_output - target) ** 2).reshape(batch_size, -1),
        dim=1,
    )
    
    ##### MIST #####
    targets_in = targets.unsqueeze(2) if targets.dim() == 4 else targets
    target_latents = vae.encode(targets_in).latent_dist.sample() * vae.config.scaling_factor
    target_latents = target_latents.permute(0, 2, 1, 3, 4)  # [B, F, C, H, W]
    target_latents = target_latents.to(memory_format=torch.contiguous_format, dtype=dtype)

    mist_loss = F.mse_loss(image_latents, target_latents, reduction="mean")

    # print(loss, mist_loss)
    return 10000 * loss - mist_loss
...
def main(args):
    ...
    for prompt, input_img, n_id in zip(prompts, input_img_list, name_list):
        ...
        if args.target_image_prompt != "":
            assert os.path.exists(args.target_image_prompt)
            target_images = [Image.open(args.target_image_prompt).resize((args.width, args.height))]
            target_images = pil_to_pt(target_images).to(accelerator.device, dtype=weight_dtype)
            target_images = target_images * 2.0 - 1.0  # [-1, 1]
            target_images.requires_grad_(False)
        else:
            target_images = None
        ...
        for epoch in range(args.num_train_epochs):
            ...
            print((perturbed_data - original_data).sum())
